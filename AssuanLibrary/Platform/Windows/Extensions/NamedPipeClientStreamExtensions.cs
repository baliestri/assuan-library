// Copyright (c) Bruno Sales <me@baliestri.dev>. Licensed under the MIT License.
// See the LICENSE file in the repository root for full license text.

using System.Buffers;
using System.Diagnostics.CodeAnalysis;
using System.IO.Pipes;

namespace AssuanLibrary.Platform.Windows.Extensions;

[ExcludeFromCodeCoverage]
internal static class NamedPipeClientStreamExtensions {
  /// <summary>
  ///   Extension methods for <see cref="NamedPipeClientStream" />.
  /// </summary>
  extension(NamedPipeClientStream namedPipeClientStream) {
    /// <summary>
    ///   Discards any available data in the TCP client receive buffer.
    /// </summary>
    public void DiscardAvailableData() {
      if (namedPipeClientStream.IsMessageComplete) {
        return;
      }

      var buffer = ArrayPool<byte>.Shared.Rent(4096);
      while (!namedPipeClientStream.IsMessageComplete) {
        _ = namedPipeClientStream.Read(buffer, 0, buffer.Length);
      }

      ArrayPool<byte>.Shared.Return(buffer, true);
    }

    /// <summary>
    ///   Discards any available data in the TCP client receive buffer asynchronously.
    /// </summary>
    /// <param name="ct">A <see cref="CancellationToken" /> to observe while waiting for the task to complete.</param>
    public async Task DiscardAvailableDataAsync(CancellationToken ct = default) {
      if (namedPipeClientStream.IsMessageComplete) {
        return;
      }

      var buffer = ArrayPool<byte>.Shared.Rent(4096);
      while (!namedPipeClientStream.IsMessageComplete) {
        _ = await namedPipeClientStream.ReadAsync(buffer, 0, buffer.Length, ct).ConfigureAwait(false);
      }

      ArrayPool<byte>.Shared.Return(buffer, true);
    }
  }
}
